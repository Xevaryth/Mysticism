...
public final class ManaHud {
    ...
    private static final ResourceLocation LAYER =
        ResourceLocation.fromNamespaceAndPath(Mysticism.MODID, "mana");
    private static final ResourceLocation TEXTURE =
        ResourceLocation.fromNamespaceAndPath(Mysticism.MODID, "textures/gui/mana_stars.png");
    ...
    private static void registerLayer(RegisterGuiLayersEvent event) {
        ...
        // Render below vanilla air so bubbles can sit above mana.
    ...
    private static void render(GuiGraphics graphics, DeltaTracker deltaTracker) {
        ...
        int visibleLayer = current <= MANA_PER_BAR
            ? 0
            : Math.max(1, (current - 1) / MANA_PER_BAR);

        int layerMana = current - visibleLayer * MANA_PER_BAR;
        if (visibleLayer > 0 && layerMana == 0) {
            layerMana = MANA_PER_BAR;
        }

        int baseU = visibleLayer > 0 ? PURPLE_BASE_U : BLUE_BASE_U;

        for (int slot = 0; slot < STARS; slot++) {
            int x = right - ICON_SIZE - slot * SLOT_SPACING;
        ...
        for (int slot = 0; slot < STARS; slot++) {
            ...
            // Sprite order is assumed:
            // full, 3/4, 1/2, 1/4 across the sheet.
